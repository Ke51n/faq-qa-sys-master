...
import com.example.tmp.SelectParagraphs;
...
//        TODO
//        System.out.println("total_counts=" + total_counts);
...
//            System.out.println("==============相似度模型返回为空，忽略相似度==============");
...
//
//        }
//        System.out.println("=======================================" + similarityScoreList.size());

        // todo
        List<Float> similarityScoreList = new ArrayList<>();
        for (int i = 0; i < total_counts; i++) {
            similarityScoreList.add(0F);
        }
//
        ...
        //TODO
//        for (Float aFloat : similarityScoreList) {
...
//        System.out.println("=========================");
        ...
        //todo
        System.out.println("=============================relevant questions=============================");
        for (MatchingDataModel mdm : matchingDataModelList) {
            System.out.println(mdm.toSimpleString());
        }
        ...
        for (int i = 0; i < dialogueConfig.getConfidenceRank().getSize() && i < total_counts; i++) {
            ...
            if (i == 0) {
                //TODO 把链接也打印
                String bestAnswer;
                String sa = matchingDataModel.getStandardAnswer();
                String url = matchingDataModel.getUrl();
                Float confidence = matchingDataModel.getConfidence();
                //归一化相关性得分
//                Float relevanceScore = matchingDataModel.getRelevanceScore();
//                matchingDataModel.setRelevanceScore(relevanceScore / 100);
                //当bestanswer 的置信度 小于 10 时，返回万金油回答
                if (confidence < 0.5) {
                    bestAnswer = CodeMsg.GENERAL_ANSWER.getMsg();
                } else {
//                    if (sa.length() > 200) {
//                        sa = sa.substring(0, 200) + "...";
//                    }
                    if (sa.length() > 500)
                        bestAnswer = SelectParagraphs.selectMostRelevantParas(question, sa);
                    bestAnswer = url == null ? sa : sa + "......\n点击查看详情" + url;
                }
